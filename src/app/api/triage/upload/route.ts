...
export async function POST(request: NextRequest) {
  ...
  if (actualBugsStored > 0) {
    ...
    // Backlog upsert — awaited so Vercel doesn't kill it before it completes.
    // (Fire-and-forget was unreliable: the function exits as soon as the response
    // is sent and any pending async work is discarded.)
    // Deduplicates bugs by user_id+bug_id across runs. On conflict we update
    // triage fields only — pm_action, detail, and first_seen_at are intentionally
    // excluded so PM verdicts and cached detail survive re-uploads.
    try {
